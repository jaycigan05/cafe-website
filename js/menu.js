// menu.js — vanilla JS only, no libraries (that's the assignment brief)
...
// the custom sort dropdown (not a native <select>, styled one)
...
sortOptions.forEach((li, i) =>
{
	...
	li.addEventListener('keydown', (e) =>
	{
		...
		{
			e.preventDefault(); chooseSortOption(li);
		}
		if (e.key === 'ArrowDown')
		{
			e.preventDefault(); (sortOptions[i + 1] || sortOptions[0]).focus();
		}
		if (e.key === 'ArrowUp')
		{
			e.preventDefault(); (sortOptions[i - 1] || sortOptions[sortOptions.length - 1]).focus();
		}
		if (e.key === 'Escape')
		{
			closeSortList(); sortTrigger.focus();
		}
	});
});
...
// renders the grid: applies filter + search + sort together
...
// top-3 podium (static ranking, doesn't change with filters)
const podiumSlots = {
...
function paintPodium(flavor, animate)
{
	...
	PODIUM_LAYOUT.forEach(({ slot, rank, medal }) =>
	{
		...
		if (!animate)
		{
			fillSlot(slotEl, item, rank, medal); return;
		}
